			 ...
			 uint8_t *sha1, const void *data, size_t len)
{
	...
	off_t offset = 0;
	off_t child_offset = from_be64(table->items[i].child);
	off_t right_child = 0;
	off_t ret = 0;
	if (child_offset) {
		/* recursion */
	...
	table->items[i].child = to_be64(child_offset);
		     ...
		     const uint8_t *sha1)
{
	while (table_offset) {
		...
		off_t child = from_be64(table->items[left].child);
		put_table(btree, table, table_offset);
		table_offset = child;
	}
	return 0;
